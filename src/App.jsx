...
const ProfileScreen = ({ history, onClose }) => {
  ...
  return (
    <div className="fadein">
      ...
      <button style={{ ...S.btnSecondary, marginTop: 8 }} onClick={onClose}>← back</button>
    </div>
  );
};
...
export default function App() {
  ...
  const getAction = useCallback(async ({ feedbackContext = null, historyOverride = history, patternsOverride = patterns } = {}) => {
    ...
    try {
      ...
      // Server normalizes all providers to { text, usedModel, upgraded }
    ...
    <div style={S.root}>
      ...
      <div style={S.topbar}>
        <button style={S.ghostBtn} onClick={() => screen !== "input" && setScreen("input")}>
          {screen !== "input" ? "← back" : <span style={{ color: "#8899bb" }}>NAO</span>}
        </button>
