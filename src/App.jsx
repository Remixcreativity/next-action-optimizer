...
const shouldAutoRemoveCompletedTask = ({ selectedFeedback, finalResult, action, task, historyAfter = [] }) => {
  ...
  const projectRepeatedFinished = parentProgress.finished >= 2 && task.task_type === 'project';
  const urgentResolved = ['urgent', 'urgent_important'].includes(task.task_type) && parentProgress.finished >= 1;
  const explicitCompletion = actionTextIncludesPermanentCompletion(action);

  // Non-habit one-offs always remove on first Done+Finished
  // Urgent/urgent_important always remove on first Done+Finished
  // Projects remove after 2+ finished outcomes or explicit closure
  const urgentImportantResolved = task.task_type === 'urgent_important';
  return Boolean(
    oneOffFinished ||
    urgentResolved ||
    urgentImportantResolved ||
    closureLike ||
    hasFinishedChip ||
    explicitCompletion ||
    projectRepeatedFinished
  );
};
