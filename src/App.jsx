...
const getPolicyControlScore = (candidate, allCandidates, history, availableMinutes, energy, state, taskLedgerSnapshot) => {
  ...
  const time_fit_penalty = availableMinutes <= 5 && ['focused_progress', 'deep_work'].includes(inferActionSize(candidate)) ? -1 : 0;
...
const chooseCandidate = (candidates, history, availableMinutes = 5, energy = "medium", taskLedgerSnapshot = {}, state = "normal") => {
  ...
  const adjustedScored = scored.map(candidate => {
    ...
    const sessionMode = getSessionMode(availableMinutes);
...
export default function App() {
  ...
  const getAction = useCallback(async ({
    ...
    try {
      ...
      // Filter candidates whose parent task is no longer available, then reject prompt/example leakage.
      const availableCandidates = filterCandidatesByAvailability(parsed.candidates, taskLedger, context);
      const validCandidates = filterGroundedCandidates(availableCandidates, taskLedger, tasks);
      const chosen = chooseCandidate(validCandidates, historyOverride, availableMinutes, energy, taskLedger, state);
      return { chosen, candidates: validCandidates };
    } catch (e) {
