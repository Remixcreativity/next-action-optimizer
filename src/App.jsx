...
return ‘t_’ + title.toLowerCase().replace(/\s+/g, ‘*’).replace(/[^a-z0-9*]/g, ‘’).slice(0, 30);
};
...
.replace(/`$/i, "") .trim(); try { return JSON.parse(cleaned); } catch {} const fencedMatch = text.match(/`(?:json)?\s*([\s\S]*?)```/i);
if (fencedMatch?.[1]) {
