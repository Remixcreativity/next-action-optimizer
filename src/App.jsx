...
const buildTaskLedger = (rawText, existingLedger = {}) => {
  ...
  lines.forEach((line, idx) => {
    ...
    // Preserve completed status for non-habit tasks — do not resurrect
    const wasCompleted = existing?.status === 'completed' && !prefixMeta.repeatable;
    const preservedStatus = wasCompleted ? 'completed' : (existing?.status || 'available');
    ...
    // Recurring/habit tasks are always available
...
const getAvailableTasks = (taskLedger, context) => {
  return Object.values(taskLedger)
    .filter(t => t.status === 'available')           // excludes completed/suppressed
    .filter(t => t.status !== 'completed')           // backup guard: never send completed tasks
...
// ─── SIMPLE COMPLETION INTENT ────────────────────────────────────────────────
// Task removal is triggered by ONE explicit signal: completion_intent = true
// No keyword guessing. No chip inference. No completion_count thresholds.

const shouldRemoveParentTask = (action, task) => {
  // Must be explicitly declared as finish attempt
  if (!action?.completion_intent) return false;
  // Never remove habits
...
const syncCompletedNonHabitTasks = (rawText = '', ledger = {}, history = []) => {
  // Only sync tasks already marked completed in ledger
  // (completion_intent path handles all new completions)
  ...
  if (!taskIds.size) return { tasks: rawText, ledger, removedCount: 0 };
  const nextTasks = removeTaskLinesByIds(rawText, taskIds);
  const existing = { ...(ledger || {}) };
  return {
    tasks: nextTasks,
    ledger: buildTaskLedger(nextTasks, existing),
    removedCount: taskIds.size,
  };
};
...
const CHIP_COLORS = {
  ...
  "finish attempt":    { bg: "#c084fc18", border: "#c084fc40", color: "#c084fc" },
};
...
const getActiveThreadBonus = (candidate, history = [], taskLedgerSnapshot = {}) => {
  ...
  if (last.feedback === 'done' && last.result === 'finished') {
    // Check if parent task is still available (not yet completed)
    const parentTask = taskLedgerSnapshot[candidate.parent_task_id];
    const taskStillActive = !parentTask || parentTask.status === 'available';
    const isHighValue = parentTask &&
      ['urgent_important', 'urgent', 'project'].includes(parentTask.task_type);

    // Strong bonus to keep working on high-value non-completed tasks
    if (taskStillActive && isHighValue) return 5;
    if (taskStillActive) return 3;
    return 0; // task completed — no thread bonus
  }
  return 0;
};
...
const getPolicyControlScore = (candidate, allCandidates, history, availableMinutes, energy, state, taskLedgerSnapshot) => {
  ...
  const active_thread_bonus = getActiveThreadBonus(candidate, history, taskLedgerSnapshot);
...
const normalizeCandidate = (candidate) => {
  ...
  return {
    ...
    completion_intent: Boolean(candidate.completion_intent ?? false),
  ...
  return (
    <div style={{ ...S.actionCard, ...(variant === "green" ? { borderColor: "#122012", background: "#0f2318" } : {}) }}>
      ...
      <div style={S.chipsRow}>
        {action.completion_intent && (
          <span style={{
            fontSize: 9, padding: "3px 8px", borderRadius: 20,
            background: "#c084fc18", border: "1px solid #c084fc40",
            color: "#c084fc", letterSpacing: "0.06em", whiteSpace: "nowrap",
          }}>
            finish attempt
          </span>
        )}
...
export default function App() {
  ...
  const handleFeedbackSubmit = async () => {
    ...
    try {
      ...
      const newEntry = {
        ...
        multiplier,
        completion_intent: currentAction.completion_intent || false,
      ...
      if (currentAction) {
        ...
        if (matchingTask) {
          ...
          if (selectedFeedback === 'done' && finalResult === 'finished') {
            ...
            // Simple rule: only remove if completion_intent = true AND non-habit
            const shouldRemove = shouldRemoveParentTask(currentAction, task);
            ...
            if (shouldRemove) {
              ...
              newEntry.parent_task_removed_reason = 'completion_intent';
              newEntry.completion_intent = true;
            } else {
  ...
  const handleBackToTasks = () => {
    // Sync completed non-habit tasks from history before returning to task list
    // This catches cases where user saw a closure action but navigated back
    // without submitting feedback (task completed via history but not via feedback)
    const synced = syncCompletedNonHabitTasks(tasks, taskLedger, history);
    if (synced.removedCount > 0) {
      setTasks(synced.tasks);
      setTaskLedger(synced.ledger);
    }
    setScreen("input");
  ...
  const handleExport = () => {
    // Build audit snapshot of task status
    const taskStatusAudit = Object.values(taskLedger).reduce((acc, t) => {
      acc[t.task_id] = {
    ...
    const completedTaskIds = Object.values(taskLedger)
    ...
    const data = {
      ...
      taskStatusAudit,
      completedTaskIds,
      currentActionParentTaskId: currentAction?.parent_task_id || null,
      availableMinutes,
      sessionMode: getSessionMode(availableMinutes),
      energy,
      context,
