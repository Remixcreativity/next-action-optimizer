...
const RESULT_OPTIONS = [
  { id: "none", label: "Nothing", pts: "+0", color: "#555" },
...
const ActionCard = ({ action, variant = "default", isFirstLoop = false }) => {
  ...
  return (
    <div style={{ ...S.actionCard, ...(variant === "green" ? { borderColor: "#122012", background: "#070d07" } : {}) }}>
      <p style={S.actionText}>{action.action}</p>
      <p style={variant === "green" ? { ...S.whyText, color: "#1e361e" } : S.whyText}>{action.why}</p>
    ...
    <div style={S.root}>
      ...
      <div style={S.topbar}>
        <button style={S.ghostBtn} onClick={() => screen !== "input" && setScreen("input")}>
          {screen !== "input" ? "← back" : <span style={{ color: "#1c1c1c" }}>NAO</span>}
        </button>
      ...
      <div style={S.card}>
        ...
        {screen === "adapted" && adaptedAction && (
          <div className="fadein">
            <div style={S.actionHead}>
              ...
              <span style={{ ...S.confPill, color: "#2a4a2a", borderColor: "#152515" }}>{stats.totalScore} pts total</span>
            </div>
        ...
        {screen === "profile" && (
          <ProfileScreen history={history} onClose={() => setScreen("input")} />
        )}
      </div>
    </div>
  );
...
const S = {
  root: { minHeight: "100vh", background: "#050505", display: "flex", flexDirection: "column", alignItems: "center", fontFamily: "'JetBrains Mono', monospace", padding: "0 18px 64px", color: "#e8e8e8" },
  ...
  ghostBtn: { background: "none", border: "none", color: "#2a2a2a", fontSize: 11, letterSpacing: "0.12em", minWidth: 52 },
  statsPill: { background: "none", border: "1px solid #141414", borderRadius: 20, padding: "5px 12px", fontSize: 10, display: "flex", alignItems: "center", gap: 6, color: "#444", letterSpacing: "0.04em" },
  pillDot: { width: 3, height: 3, borderRadius: "50%", background: "#2a2a2a" },
  errorBanner: { width: "100%", maxWidth: 480, background: "#1a0808", border: "1px solid #3a1414", borderRadius: 8, padding: "10px 14px", marginBottom: 8, display: "flex", alignItems: "center", justifyContent: "space-between" },
  ...
  statsPanel: { width: "100%", maxWidth: 480, background: "#080808", border: "1px solid #111", borderRadius: 10, padding: "18px 20px", marginBottom: 10 },
  ...
  statLbl: { fontSize: 9, color: "#2a2a2a", letterSpacing: "0.12em" },
  barTrack: { height: 2, background: "#111", borderRadius: 2, overflow: "hidden" },
  barFill: { height: "100%", background: "linear-gradient(90deg, #2a6a2a, #4ade80)", borderRadius: 2, transition: "width 0.7s ease" },
  profileBtn: { marginTop: 14, width: "100%", padding: "10px", background: "#080808", color: "#444", border: "1px solid #111", borderRadius: 8, fontSize: 10, letterSpacing: "0.12em" },
  card: { width: "100%", maxWidth: 480, paddingTop: 8 },
  eyebrow: { fontSize: 10, color: "#252525", letterSpacing: "0.3em", textTransform: "uppercase", marginBottom: 16 },
  textarea: { width: "100%", minHeight: 164, background: "#080808", border: "1px solid #141414", borderRadius: 8, color: "#777", fontSize: 13, lineHeight: 2, padding: "16px 18px", resize: "none", transition: "border-color 0.2s", marginBottom: 10 },
  contextBox: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 },
  contextLabel: { fontSize: 9, color: "#252525", letterSpacing: "0.2em", textTransform: "uppercase", marginBottom: 7 },
  contextOptions: { display: "flex", gap: 6 },
  contextBtn: { padding: "7px 9px", background: "#080808", border: "1px solid #141414", borderRadius: 6, fontSize: 10 },
  returningNote: { fontSize: 10, color: "#1c1c1c", marginBottom: 14, letterSpacing: "0.04em" },
  btnPrimary: { width: "100%", padding: "15px", background: "#e8e8e8", color: "#050505", border: "none", borderRadius: 8, fontSize: 12, letterSpacing: "0.18em", transition: "opacity 0.2s", marginTop: 8 },
  btnSecondary: { width: "100%", padding: "15px", background: "#080808", color: "#444", border: "1px solid #141414", borderRadius: 8, fontSize: 12, letterSpacing: "0.18em", marginTop: 8 },
  actionHead: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 },
  actionCard: { background: "#080808", border: "1px solid #111", borderRadius: 10, padding: "28px", marginBottom: 14 },
  actionText: { fontSize: 20, lineHeight: 1.5, color: "#e8e8e8", fontWeight: 400, marginBottom: 18 },
  whyText: { fontSize: 11, color: "#252525", lineHeight: 1.65, marginBottom: 16 },
  actionFooter: { display: "flex", alignItems: "center", justifyContent: "space-between" },
  timePill: { display: "inline-block", fontSize: 10, color: "#2a2a2a", background: "#0d0d0d", border: "1px solid #141414", borderRadius: 20, padding: "4px 10px" },
  confPill: { fontSize: 9, color: "#1e3a1e", background: "#080f08", border: "1px solid #112011", borderRadius: 20, padding: "4px 10px", letterSpacing: "0.08em" },
  potentialPts: { fontSize: 10, color: "#1e1e1e" },
  tagRow: { display: "flex", flexWrap: "wrap", gap: 6, marginTop: 14 },
  tagChip: { fontSize: 9, color: "#3a3a3a", background: "#0c0c0c", border: "1px solid #141414", borderRadius: 4, padding: "2px 7px", letterSpacing: "0.05em" },
  selectionReason: { marginTop: 10, fontSize: 10, lineHeight: 1.5 },
  selectionMeta: { display: "flex", gap: 6, fontSize: 10, color: "#1e1e1e", marginBottom: 8 },
  hintText: { fontSize: 10, color: "#1a1a1a", textAlign: "center", letterSpacing: "0.1em", margin: "10px 0 4px" },
  prevActionText: { fontSize: 11, color: "#252525", marginBottom: 20, lineHeight: 1.55, fontStyle: "italic" },
  feedGrid: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 7, marginBottom: 12 },
  feedBtn: { padding: "13px 14px", background: "#0a0a0a", border: "1px solid #141414", borderRadius: 8, fontSize: 11, display: "flex", alignItems: "center", gap: 8, textAlign: "left", transition: "all 0.15s" },
  resultRow: { display: "flex", alignItems: "center", gap: 8, marginBottom: 12, flexWrap: "wrap" },
  resultLbl: { fontSize: 10, color: "#2a2a2a", letterSpacing: "0.15em" },
  resultBtn: { padding: "8px 12px", background: "#080808", border: "1px solid #141414", borderRadius: 6, fontSize: 11, display: "flex", alignItems: "center", transition: "all 0.15s" },
  textInput: { width: "100%", padding: "13px 16px", background: "#080808", border: "1px solid #141414", borderRadius: 8, color: "#555", fontSize: 12, marginBottom: 4, transition: "border-color 0.2s" },
  trail: { borderTop: "1px solid #0d0d0d", paddingTop: 12, marginTop: 4, marginBottom: 16 },
  trailRow: { display: "flex", justifyContent: "space-between", alignItems: "center", padding: "7px 0", borderBottom: "1px solid #090909" },
  trailAction: { fontSize: 10, color: "#1e1e1e", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: "68%" },
  trailFeed: { fontSize: 9, color: "#1a1a1a", letterSpacing: "0.08em" },
  profileHeader: { marginBottom: 20 },
  profileSubtle: { fontSize: 11, color: "#2a2a2a", letterSpacing: "0.05em" },
  profileSection: { marginBottom: 24 },
  profileSectionTitle: { fontSize: 9, color: "#3a3a3a", letterSpacing: "0.25em", textTransform: "uppercase", marginBottom: 12 },
  insightCard: { background: "#080808", border: "1px solid #111", borderRadius: 8, padding: "14px 16px", marginBottom: 8 },
  insightDim: { fontSize: 9, color: "#2a2a2a", letterSpacing: "0.2em", textTransform: "uppercase", marginBottom: 10 },
  ...
  insightVal: { color: "#888", flex: 1 },
  insightRate: { color: "#666", fontSize: 11, fontWeight: 500 },
  insightCount: { color: "#2a2a2a", fontSize: 9 },
  energyRow: { display: "flex", alignItems: "center", gap: 10, marginBottom: 8 },
  energyLabel: { fontSize: 11, color: "#666", width: 100 },
  energyBarTrack: { flex: 1, height: 4, background: "#111", borderRadius: 2, overflow: "hidden" },
  energyBarFill: { height: "100%", background: "linear-gradient(90deg, #2a6a2a, #4ade80)", borderRadius: 2 },
  energyRate: { fontSize: 10, color: "#888", width: 40, textAlign: "right" },
  profileEmpty: { background: "#080808", border: "1px solid #111", borderRadius: 8, padding: "32px 20px", textAlign: "center", marginBottom: 16 },
  profileEmptyText: { fontSize: 12, color: "#444", lineHeight: 1.6 },
};
