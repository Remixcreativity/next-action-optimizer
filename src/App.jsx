...
const chooseCandidate = (candidates, history, availableMinutes = 5, energy = "medium", taskLedgerSnapshot = {}) => {
  ...
  // 30m/high-energy adjustment — use taskLedger for real task type
  const adjustedScored = scored.map(candidate => {
    let boost = 0;
    const tags = candidate.tags || {};
    // Use parent task ledger type (accurate) rather than tags.task_type (AI-generated, may differ)
    ...
    if (availableMinutes >= 30 && energy === 'high') {
      if (['project', 'urgent_important'].includes(ledgerType)) boost += 0.25;
      if ((candidate.estimated_minutes || 5) >= 15) boost += 0.15;
      if (tags.size === 'medium') boost += 0.10;
      if ((candidate.estimated_minutes || 5) <= 10) boost -= 0.20;
      if (ledgerType === 'habit' || tags.task_type === 'health') boost -= 0.15;
      if (tags.energy_required === 'low') boost -= 0.10;
    }

    return {
      ...candidate,
      predicted_finish_probability: Math.max(0.05, Math.min(0.95, (candidate.predicted_finish_probability || 0.5) + boost)),
    };
  });
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      if (saved.taskLedger) setTaskLedger(saved.taskLedger);
    }
    setHydrated(true);
  }, []);
  ...
  const getAction = useCallback(async ({
    ...
    try {
      ...
      const response = await fetch("/api/next-action", {
        ...
        body: JSON.stringify({
          tasks: formatTasksForServer(getAvailableTasks(taskLedger, context)),
          availableTasks: getAvailableTasks(taskLedger, context),
          rawTasks: tasks,
      ...
      // Filter candidates whose parent task is no longer available
      const validCandidates = filterCandidatesByAvailability(parsed.candidates, taskLedger, context);
      const chosen = chooseCandidate(validCandidates, historyOverride, availableMinutes, energy, taskLedger);
  ...
  const handleFeedbackSubmit = async () => {
    ...
    const score = weightedScore; // score shown to user = weighted
            ...
            <div style={S.contextBox}>
              <div>
                ...
                <div style={S.contextOptions}>
                  {TIME_OPTIONS.map(t => (
                    <button key={t} style={{ ...S.contextBtn, borderColor: availableMinutes === t ? "#e8e8e8" : "#2a3a5c", color: availableMinutes === t ? "#e8e8e8" : "#8899bb" }} onClick={() => setAvailableMinutes(t)}>{t}m</button>
                  ))}
                </div>
              </div>
              <div>
