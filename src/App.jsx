...
const shouldAutoRemoveCompletedTask = ({ selectedFeedback, finalResult, action, task }) => {
  // Parent-task removal is intentionally explicit.
  // Done + Finished means the suggested action was completed.
  // Only completion_intent=true means the whole parent task should be removed.
...
const FINISH_ELIGIBLE_TYPES = ['one_off', 'urgent', 'urgent_important'];
...
const shouldForceFinishAttempt = (candidate, activeThread, taskLedgerSnapshot) => {
  ...
  if (!task || task.repeatable || task.task_type === 'habit') return false;
  // Only completable task types — never project or habit
  if (!FINISH_ELIGIBLE_TYPES.includes(task.task_type)) return false;
...
const ActionCard = ({ action, variant = "default", isFirstLoop = false }) => {
  ...
  return (
    <div style={{ ...S.actionCard, ...(variant === "green" ? { borderColor: "#122012", background: "#0f2318" } : {}) }}>
      ...
      {([...(action.reason_chips || []), ...(action.completion_intent ? ["finish attempt"] : [])].length > 0) && (
        <div style={S.chipsRow}>
          {[...(action.reason_chips || []), ...(action.completion_intent ? ["finish attempt"] : [])].map((chip, i) => {
  ...
  const handleFeedbackSubmit = async () => {
    ...
    try {
      ...
      const newEntry = {
        ...
        completion_intent: currentAction.completion_intent === true,
  ...
  const handleExport = () => {
    const data = {
      ...
      policy_config: {
        ...
        removal_rule: 'completion_intent_true_plus_done_finished_non_habit',
      },
    };
