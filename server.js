...
const buildMessages = ({ tasks, availableTasks, history, feedbackContext, availableMinutes, sessionMode, energy, activeThread }) => {
  ...
  const getAdaptationRule = (t) => {
    ...
    if (f === "done" && r === "finished") {
      ...
      if (["project"].includes(t.parent_task_type || "")) return "→ PROJECT MILESTONE: " + fc + " steps done on this project. Suggest: Close this mini-loop for this task — write down the exact next implementation step and save it. completion_intent=false. Do NOT say remove from list.";
...
app.post("/api/next-action", async (req, res) => {
  try {
    ...
    console.log("[NAO DEBUG] activeThread received:", JSON.stringify(activeThread));
    console.log("[NAO DEBUG] availableTasks received:", (availableTasks||[]).map(t => t.task_id + " comp=" + (t.completion_count||0)));
    ...
    console.log("[NAO DEBUG] Full response:", raw);
